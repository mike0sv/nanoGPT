...
def checkout():
    if not os.path.exists(repo_path):
        repo = git.Repo.clone_from("https://github.com/iterative/mlem.ai/", repo_path)
    else:
...
def main():
    checkout()
    docs_model = load("mlem_model")

    for filename in tqdm(glob.glob(repo_path + "/content/docs/**/*.md")):
